...
Session update: runner is now exposed on sync/async clients, async runner example added, and API list updated with root; next focus is extending runner features (streaming + schema helpers/introspection) and then moving on to remaining resource modules with async parity.
