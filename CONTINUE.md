...
Session update: added chat message audio + URL citation annotation types, plus transcription usage duration support and tests; next focus is filling image/audio content blocks for responses (structured pieces) and any remaining schemas from `docs/api/schemas.md` before moving on to resource parity.
