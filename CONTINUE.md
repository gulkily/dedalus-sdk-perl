...
Session update: runner streaming support added for sync/async (stream returns `Dedalus::Stream` with final result on `->result`); next focus is schema helpers/introspection and then moving on to remaining resource modules with async parity.
